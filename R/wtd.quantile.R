wtd.quantile <- function (x, weights = NULL, probs = c(0, 0.25, 0.5, 0.75, 1), 
                          ...
                          na.rm = TRUE) {
  if (!length(weights) | unique(length(weights)) == 1)
    return(quantile(x, probs = probs, type = 6, na.rm = na.rm))
  type <- match.arg(type)
  if (any(probs < 0 | probs > 1)) 
    stop("Probabilities must be between 0 and 1 inclusive")
  nams <- paste(format(round(probs * 100, if (length(probs) > 1) 2 - log10(diff(range(probs))) else 2)), "%", sep = "")
  ...
  if (type == "quantile") {
    ...
    high <- pmin(low + 1, n)
    order <- order%%1
    allq <- approx(cumsum(wts), x, xout = c(low, high), method = "constant", f = 1, rule = 2)$y
    k <- length(probs)
